...
>
> Why vendor locally: the previous CDN load (`cdn.jsdelivr.net` with `unpkg.com` fallback — see [#61](https://github.com/BrennenJohnston/braille-card-and-cylinder-stl-generator/pull/61) for context on the previous npm-dep removal) was silently blocked by Firefox Enhanced Tracking Protection (Strict), Safari content blockers, uBlock Origin, corporate proxies, and offline use — causing cylinder generation to fail on Firefox 114+ and Safari 15+ users while Chrome users with a clean profile never saw the issue.
